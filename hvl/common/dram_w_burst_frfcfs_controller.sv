// SDRAM timing model
// Based on MT48LC64M4A2-75, adapted for this processor testbench
// Limitations:
//   Refresh not modeled
...
#(
    ...
    string memfile;
    initial begin
        if (!$value$plusargs("OOOCPU_BRAM_0_ON_X=%d", BRAM_0_ON_X)) begin
            void'($value$plusargs("BRAM_0_ON_X_ECE411=%d", BRAM_0_ON_X));
        end
        if (!$value$plusargs("OOOCPU_CLOCK_PERIOD_PS=%d", CLOCK_PERIOD_PS)) begin
            void'($value$plusargs("CLOCK_PERIOD_PS_ECE411=%d", CLOCK_PERIOD_PS));
        end
        if (!$value$plusargs("OOOCPU_MEMLST=%s", memfile)) begin
            void'($value$plusargs("MEMLST_ECE411=%s", memfile));
        end
