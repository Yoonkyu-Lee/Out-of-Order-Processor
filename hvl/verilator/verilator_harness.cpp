...
static inline bool get_bool_plusarg_alias(unique_ptr<VerilatedContext> const& contextp, string primary, string legacy) {
    return get_bool_plusarg(contextp, primary) || get_bool_plusarg(contextp, legacy);
}
...
static inline uint64_t get_int_plusarg_alias(unique_ptr<VerilatedContext> const& contextp, string primary, string legacy) {
    if (get_bool_plusarg(contextp, primary)) {
        return get_int_plusarg(contextp, primary);
    }
    return get_int_plusarg(contextp, legacy);
}
...
int main(int argc, char** argv, char** env) {
    ...
    try {
        clk_half_period = get_int_plusarg_alias(contextp, "OOOCPU_CLOCK_PERIOD_PS", "CLOCK_PERIOD_PS_ECE411") / 2;
    ...
    tfp->open("dump.fst");
    bool dump_all = !get_bool_plusarg_alias(contextp, "OOOCPU_NO_DUMP_ALL", "NO_DUMP_ALL_ECE411");
